...
function actualizarEstadoVentanas() {
  ...
  // Solo para referencia visual, NO para bloquear
...
function cambiarTab(tabId) {
  ...
  if (tabId === 'ciclo1') {
    ...
    // Configurar eventos de los tabs de grupos
    document.querySelectorAll('.esp-grupo-tab').forEach(tab => {
      tab.onclick = () => {
        ...
        if (panel && GRUPOS_EQUIPOS[grupoActivo]) {
          ...
          // IMPORTANTE: Reconfigurar eventos después de actualizar el panel
...
function setupEventListeners() {
  ...
  // ========== SELECTORES DE FINALISTAS (CICLO 2) ==========
